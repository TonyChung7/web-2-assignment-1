...
function renderCharacterList(characterArray) {
...
function renderCharactersListBelowAge(characterArray, ageThreshold) {
  const targetList = document.getElementById("age-filter-list");
  const filteredCharactersArray = characterArray.filter(
    (character) => character.age < ageThreshold
  );
  if (filteredCharactersArray.length === 0) {
    const li = document.createElement("li");
    li.textContent = `No characters found below age ${ageThreshold}.`;
    li.classList.add("empty-list");
    targetList.appendChild(li);
    return;
  }
  filteredCharactersArray.forEach((character) => {
    const li = document.createElement("li");
    li.textContent = character.name;
    targetList.appendChild(li);
  });
}
renderCharactersListBelowAge(characters, 20);
...
function renderCharacterListWithErrors(characterArray, targetListId, errorMesId) {
  const targetList = document.getElementById(targetListId);
  const errorMes = document.getElementById(errorMesId);
  characterArray.forEach((character) => {
    if (!character.name) {
      console.error(`Error: Character with id ${character.id} is missing the name property.`);
      const errorP = document.createElement("p");
      errorP.textContent = `Error: Character with id ${character.id} is missing the name property.`;
      errorP.classList.add("error-message");
      errorMes.appendChild(errorP);
      return;
    }
    const li = document.createElement("li");
    li.textContent = character.name;
...
renderCharacterListWithErrors(characters, "error-handling-list", "error-messages");
...
const brokenCharacters = [
  { id: 11, age: 25 },
  { id: 12, name: "Hello Kitty", age: 35 },
  { id: 13, age: 40 },
  { id: 14, name: "GoKu", age: 28 },
];

renderCharacterListWithErrors(brokenCharacters, "broken-array-list", "broken-array-errors");
